...
REGISTER_OP("HcomAllGather")
    .Input("input: T")
    .Output("output: T")
    .Attr("T: {int8, int16, int32, float16, float32, int64, uint64, uint8, uint16, uint32, float64}")
    .Attr("group: string")
    .Attr("rank_size: int")
    .SetIsStateful()
    .SetShapeFn([](shape_inference::InferenceContext *c) {
...
REGISTER_OP("HcomGatherAllToAllV")
    ...
    .Attr("dtype: {int8, int16, int32, float16, float32, int64, uint64, uint8, uint16, uint32, float64}")
