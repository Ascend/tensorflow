...
bool IsLazyRecompile(std::map<std::string, std::string> &graph_options) {
  return (graph_options["dynamic_input"] == "1" && graph_options["dynamic_graph_execute_mode"] == "lazy_recompile") ||
...
Status OMPartitionSubgraphsPass::ProcessGraph(std::unique_ptr<Graph> *graph, FunctionLibraryDefinition *func_lib,
  ...
  if (OMSplitter::IsLazyRecompile(graph_options) && (iterations_per_loop > 1)) {
