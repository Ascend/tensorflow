...
Status OMSubGraphToFunctionDef(const Graph &graph, const string &name, FunctionDef *fdef) {
  ...
  for (Node const *node : graph.op_nodes()) {
    ...
    *nodeDef = node->def();
    if (!node->assigned_device_name().empty()) {
      nodeDef->set_device(node->assigned_device_name());
    }
      ...
      if (edge->IsControlEdge()) {
        ...
        if (inEdges.size() <= dst_input) {
          try {
            ...
            return errors::InvalidArgument("inEdges resize is failed, resize is %u", dst_input + 1);
          }
        }
  ...
  for (int i = 0; i < fdef->signature().input_arg_size(); ++i) {
    ...
    if (inputName.empty()) {
      return errors::InvalidArgument("Missing input ", i, " to function ", name);
    }
  }
  ...
  for (int r = 0; r < fdef->signature().output_arg_size(); ++r) {
    ...
    if (retName.empty()) {
      return errors::InvalidArgument("Missing output ", r, " to function ", name);
    }
...
bool IsWhiteListSupport(const string &op_name, bool mix_compile_mode, const string &node_name,
  ...
  bool ans = (identifier->IsNpuSupported(op_name, node_name)) && !EndsWith(op_name, suffix_op) &&
      !EndsWith(op_name, suffix_op_v2) && (support_const || !(op_name == "Const")) && !(op_name == "_Arg") &&
...
bool IsWithoutNpuScope(const NodeDef &node_def) {
  ...
  if (node_def.attr().count(ATTR_VALUE_SCOPE_NAME)) {
    return node_def.attr().at(ATTR_VALUE_SCOPE_NAME).b();
  }
  return false;
}
  ...
  for (NodeDef node_def : func_def->node_def()) {
    ...
    for (const auto &item : node_def.attr()) {
      if (item.second.has_func()) {
        if (!IsNpuSupportingFunc(item.second.func().name(), func_lib, depth + 1)) {
          return false;
        }
      }
    }
...
bool IsNpuSupportingFunc(const Node *node, const FunctionLibraryDefinition *func_lib, int depth) {
  for (const auto &it : node->attrs()) {
    if (it.second.has_func()) {
      ...
      if (!IsNpuSupportingFunc(func_name, func_lib, depth)) {
        return false;
      }
    }
  }
...
bool IsNpuSupportingNode(const NodeDef &node_def, bool mix_compile_mode, const FunctionLibraryDefinition *func_lib,
                         bool support_const) {
  if (IsWithoutNpuScope(node_def)) {
    return false;
  }
  if (IsWhiteListSupport(node_def.op(), mix_compile_mode, node_def.name(), support_const)) {
    return true;
  }
  if (IsNpuSupportingFunc(node_def.op(), func_lib, 0)) {
    return true;
  }
  return false;
}
...
int FindNodesInPaths(Node *op_head, NodeSet &ops_tail, NodeSet &ops_save) {
  if (!op_head || ops_tail.count(nullptr) > 0) {
    return 0;
  }
  ...
  while (!stack.empty()) {
    ...
    if (!cur_node) {
      ...
      if (path.size() >= kLegalPathSize) {
        seen[path[path.size() - kLegalPathSize]].erase(path.back());
      }
      path.pop_back();
      continue;
    }
    ...
    if (path.size() >= kLegalPathSize) {
      seen[path[path.size() - kLegalPathSize]].insert(path.back());
    }
    stack.push_back(nullptr);

    if (ops_tail.count(cur_node) > 0 || ops_save.count(cur_node) > 0) {
      for (auto node : path) {
        ops_save.insert(node);
      }
      continue;
    }
    ...
    for (auto out_node : cur_node->out_nodes()) {
      ...
      if (seen.insert(NodeMap::value_type(out_node, empty)).second || seen[out_node].size() < outputs_size) {
  ...
  for (auto &one_graph_iop : all_graph_iop) {
    for (auto &iop : one_graph_iop) {
      ...
      for (auto node : graph.nodes()) {
        if (node->name() == iop.first) {
          op_head = node;
        }
        if (iop.second.count(node->name()) > 0) {
          ops_tail.insert(node);
        }
      }
...
Status FindNpuSupportCandidates(const Graph &graph, OrderedNodeSet *candidates,
  ...
  for (Node *node : sortedNodes) {
    ...
    if (enableDP &&
      ...
      if (node->type_string() == "AdpGetNext") {
        candidates->insert(node);
      }
    } else {
  ...
  if (mix_compile_mode) {
    ...
    for (auto it : outSet) {
      ...
      if (!cfInfo.frame_name.empty()) {
        unsupportedFrames.insert(cfInfo.frame_name);
      }
    ...
    for (auto edge : node->out_edges()) {
      ...
      if (!edge->IsControlEdge()) {
        ...
        if (IsRefType(dtypeDst) && candidates->count(edge->dst()) > 0) {
          ...
          ADP_LOG(INFO) << "Remove node : " << edge->dst()->name()
                        << " from candidates, because of node : " << node->name() << " REF input.";
          continue;
        }
        if (dtypeDst == DT_STRING || dtypeDst == DT_RESOURCE) {
          if (edge->dst()->type_string() == "Assert") {
            continue;
          }
          if (node->type_string() == "Const") {
            continue;
    ...
    for (auto edge : node->in_edges()) {
      ...
      if (!edge->IsControlEdge()) {
        ...
        if (IsRefType(dtypeDst) && candidates->count(edge->src()) > 0) {
          ...
          ADP_LOG(INFO) << "Remove node : " << edge->dst()->name()
                        << " from candidates, because of node : " << node->name() << " REF Output.";
          continue;
        }
        if (dtypeDst == DT_STRING || dtypeDst == DT_RESOURCE) {
...
bool NodeIsCandidateForClustering(Node *node, OrderedNodeSet *candidates) {
...
Status MergeSubgraphsInNewWay(std::vector<std::pair<string, int>> &sortedCluster, OrderedNodeSet &npuSupportCandidates,
  ...
  int64 startTime = InferShapeUtil::GetCurrentTimestap();
  if (sortedCluster.size() < MIN_CLUSTER_SIZE) {
    return Status::OK();
  }
  ...
  while (mergedClusters.size() < sortedCluster.size()) {
    ...
    clusterindex++;
    if (clusterindex >= sortedCluster.size()) {
      break;
    }
  }
...
Status MergeSubgraphs(std::vector<std::pair<string, int>> &sortedCluster, OrderedNodeSet &npuSupportCandidates,
  ...
  for (uint32_t i = 1; i < sortedCluster.size(); i++) {
    ...
    if (canMerge && mergedClusters.count(name) == 0) {
      mergedClusters.insert(name);
    }
  }
...
Status MarkForPartition(std::unique_ptr<Graph> *graph_in, int &clusterNum, bool mix_compile_mode, int graph_num,
  ...
  bool is_set_lazy_recompile =
      graph_options["dynamic_input"] == "1" && graph_options["dynamic_graph_execute_mode"] == "lazy_recompile";
  ...
  for (auto node : graph->nodes()) {
    ...
    try {
      ...
      return errors::Internal("make shared failed");
    }
    ...
    if (node_attr_value != nullptr) {
      ...
      for (const auto &startNodeName : startNodeVec) {
        cluster_map[node]->start_nodes_name.insert(startNodeName);
      }
    }
  }
      ...
      if (cycles.HasEdge(src_index, dst_index) && cycles.ContractEdge(src_index, dst_index)) {
        if (job != "localhost") {
          ...
          for (const auto &src_start_name : cluster_src->start_nodes_name) {
            if (std::any_of(
                    cluster_dst->start_nodes_name.begin(), cluster_dst->start_nodes_name.end(),
          ...
          if (mix_compile_mode) {
            ...
            if (!canReach && job != "localhost") {
              ...
              for (const auto &cluster_item : cluster_map) {
                ...
                if (cluster_src != nullptr && cluster_dst != nullptr) {
                  break;
                }
              }
              bool find_same_start = false;
  ...
  std::transform(clusterInfo.begin(), clusterInfo.end(), std::back_inserter(sortedCluster),
...
class OMSplitter {
 ...
 private:
  ...
  int FindOutputSlotOfEdgeSrc(const string &srcSubgraphId, const Edge *edge) const;
  ...
  Status FindOutputImageOfEdgeDst(const string &dstSubgraphId,
                                  const std::unordered_map<const Node *, Node *> &nodeImages,
  ...
  // it is a slot on a node in the output graph.
  int FindOutputSlotOfEdgeDst(const string &dstSubgraphId, const Edge *edge) const;
...
Node *OMSplitter::Subgraph::GetGEOpNodeForInputs() const {
  return GEOpNodeInputs_;
}

Node *OMSplitter::Subgraph::GetGEOpNodeForOutputs() const {
  return GEOpNodeOutputs_;
}
...
Graph *OMSplitter::Subgraph::GetGraph() const {
  return graph_.get();
}
...
bool OMSplitter::Subgraph::isIsolatedSubgraph() const {
  return false;
}
...
bool IsInSubgraph(const string &subgraphId) {
  return !subgraphId.empty();
}
...
Status OMSplitter::SplitIntoSubgraphs(uint32_t &subgraphNum) {
  ...
  for (auto &entry : subgraphs_) {
    allSubgraphNames.push_back(entry.first);
  }
  for (string &subgraphName : allSubgraphNames) {
    ...
    if (subgraph.isIsolatedSubgraph()) {
      ...
      for (Node *node : graph_in_->op_nodes()) {
        string subgraphId;
        TF_RETURN_IF_ERROR(GetSubgraphIdAttr(node, &subgraphId));
        if (IsInSubgraph(subgraphId) && subgraphId == subgraphName) {
          node->ClearAttr(groupAttribute_);
        }
      }
    }
  }
...
Status OMSplitter::AddGEOpNodes(const std::unordered_map<const Node *, Node *> &nodeImages, Graph *graphOut) {
  for (auto &subgraphEntry : subgraphs_) {
    TF_RETURN_IF_ERROR(subgraphEntry.second.AddGEOpNode(nodeImages, graphOut));
  }
  return Status::OK();
}
...
int OMSplitter::FindOutputSlotOfEdgeSrc(const string &srcSubgraphId, const Edge *edge) const {
  if (IsInSubgraph(srcSubgraphId)) {
...
Status OMSplitter::FindOutputImageOfEdgeDst(const string &dstSubgraphId,
                                            const std::unordered_map<const Node *, Node *> &nodeImages,
                                            const Node *originalDstNode, Node **dstImage) {
  if (IsInSubgraph(dstSubgraphId)) {
...
int OMSplitter::FindOutputSlotOfEdgeDst(const string &dstSubgraphId, const Edge *edge) const {
  if (IsInSubgraph(dstSubgraphId)) {
...
Status OMPartitionSubgraphsInFunctions(string groupAttribute, std::unique_ptr<Graph> *graph, const string &graph_format,
  ...
  OMSplitter omsplitter(std::move(groupAttribute), graph_in, std::move(npu_optimizer_options), std::move(pass_options),
                        std::move(graph_options));
  ...
  if (enable_dp == "1") {
    ...
    if (NpuAttrs::GetUseAdpStatus(queue_name)) {
      ...
      for (const Edge *e : node->out_edges()) {
        graph_in->AddEdge(adp_getnext_node, e->src_output(), e->dst(), e->dst_input());
      }
...
Status OMPartitionSubgraphsPass::ProcessGraph(std::unique_ptr<Graph> *graph, FunctionLibraryDefinition *func_lib,
  ...
  if (getnext_node_count > 1) {
    ADP_LOG(FATAL) << "dynamic dims func can not support graph with " << getnext_node_count << " IteratorGetNext node.";
    LOG(FATAL) << "dynamic dims func can not support graph with " << getnext_node_count << " IteratorGetNext node.";
  }
  std::map<std::string, std::string> graph_options;
  ...
  for (Node *node : graph_in->op_nodes()) {
    ...
    if (status.ok() && !node_format_value.empty()) {
      if (graph_format_value.empty()) {
        graph_format_value = node_format_value;
      }
    }
  }
...
Status OMPartitionSubgraphsPass::CopyVarsBetweenGeOp(Graph *graph) const {
  ...
  for (Node *node : graph->op_nodes()) {
    if (node->type_string() == "VariableV2" || node->type_string() == "VarHandleOp") {
      std::copy(node->out_edges().begin(), node->out_edges().end(), std::back_inserter(varEdges));
    }
  }
  for (auto varEdge : varEdges) {
    ...
    if ((IsRefType(dtypeDst) || dtypeDst == DT_RESOURCE) && srcSubgraphId != dstSubgraphId) {
      ...
      graph->AddEdge(nodeCopy, varEdge->src_output(), varEdge->dst(), varEdge->dst_input());
...
Status OMPartitionSubgraphsPass::CopyConstBetweenGeOp(Graph *graph) const {
  for (Node *node : graph->op_nodes()) {
    if (node->IsConstant()) {
      ...
      std::copy(node->out_edges().begin(), node->out_edges().end(), std::back_inserter(edges));
      for (auto edge : edges) {
        REQUIRES_NOT_NULL(edge);
        REQUIRES_NOT_NULL(edge->src());
        REQUIRES_NOT_NULL(edge->dst());
        ...
        if (srcSubgraphId != dstSubgraphId) {
          ...
          graph->AddEdge(nodeCopy, edge->src_output(), edge->dst(), edge->dst_input());
          graph->RemoveEdge(edge);
          graph->AddControlEdge(edge->src(), nodeCopy, false);
        }
      }
    }
  }
...
Status OMPartitionSubgraphsPass::AccumulateNFusion(Graph *graph_in, Node *node) const {
  ...
  for (auto out_edge : node->out_edges()) {
    if (out_edge->IsControlEdge()) {
      graph_in->AddControlEdge(accumulate_node, out_edge->dst());
    } else {
      graph_in->AddEdge(accumulate_node, 0, out_edge->dst(), out_edge->dst_input());
    }
  }
