...
def _refresh_model_dir_and_session_config(config, model_dir):
  ...
  if config is None or not isinstance(config, NPURunConfig):
    raise ValueError(
        'config must be an instance of `NPURunConfig`, but provided %s.' % config)
...
def model_to_npu_estimator(keras_model=None,
  ...
  estimator = tf.keras.estimator.model_to_npu_estimator(keras_model)
  ...
  Args:
    ...
    custom_objects: Dictionary for custom objects.
    model_dir: Directory to save `NPUEstimator` model parameters, graph, summary
      files for TensorBoard, etc.
    checkpoint_format: Sets the format of the checkpoint saved by the NPUEstimator
      ...
      `model_to_npu_estimator` is only supported by Functional and Sequential
      models.
    config: `NPURunConfig` to config `NPUEstimator`.
    job_start_file: The path of the job start file. Cannot be `None`.

  Returns:
    An NPUEstimator from given keras model.
  ...
  if not hasattr(keras_model, 'optimizer') or not keras_model.optimizer:
    raise ValueError(
        ...
        'before calling `model_to_npu_estimator()`.')
  ...
  if _any_weight_initialized(keras_model):
    ...
    if config.session_config.HasField('gpu_options'):
      logging.warning(
          ...
          'The _session_config passed to model_to_npu_estimator will not be used.')
  else:
