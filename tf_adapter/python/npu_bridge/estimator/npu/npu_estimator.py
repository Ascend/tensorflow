...
class NPUEstimator(estimator_lib.Estimator):
    ...
    def __load_experimental_config(self, config, custom_op):
        ...
        if config._experimental_config is None:
            ...
            if config._experimental_config._accelerate_train_mode is not None:
                custom_op.parameter_map[
                    "experimental_accelerate_train_mode"].s = tf.compat.as_bytes(
                    config._experimental_config._accelerate_train_mode)
    ...
    def __load_graph_optimizers(self, config):
        ...
        custom_op.parameter_map["jit_compile"].s = tf.compat.as_bytes(config._jit_compile)
