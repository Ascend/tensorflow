...
#include "tf_adapter/kernels/aicore/dynamic_gruv2_grad_ops.cc"
...
TEST(DynamicGruV2GradTest, TestDynamicGruV2Grad) {
  DataTypeSlice input_types({DT_HALF, DT_HALF, DT_HALF, DT_FLOAT, DT_FLOAT,
  ...
  DynamicGruV2GradOP<int> dynamic_gruv2_grad(context);
  OpKernelContext *ctx = nullptr;
  dynamic_gruv2_grad.Compute(ctx);
  dynamic_gruv2_grad.IsExpensive();
  delete device;
  delete node_def;
  delete op_def;
  delete context;
...
TEST(DynamicGruV2GradOpTest, TestDynamicGruV2GradShapeInference) {
  const OpRegistrationData *reg;
  TF_CHECK_OK(OpRegistry::Global()->LookUp("DynamicGruV2Grad", &reg));
