...
TEST(HostFeatureMappingTest, HostFeatureMappingExportTestShapeInference) {
  const OpRegistrationData *reg;
  TF_CHECK_OK(OpRegistry::Global()->LookUp("FeatureMappingExport", &reg));
  OpDef op_def = reg->op_def;
  NodeDef def;
  std::vector<std::string> table_name_list = {"table_name1"};
  TF_CHECK_OK(NodeDefBuilder("dummy", &op_def)
                  .Input(FakeHostInputStub(DT_STRING))
                  .Attr("table_name_list", table_name_list)
                  .Finalize(&def));
  shape_inference::InferenceContext c(0, &def, op_def, {THostShape({1})}, {}, {}, {});
  TF_CHECK_OK(reg->shape_inference_fn(&c));
}

TEST(HostFeatureMappingTest, HostFeatureMappingImportTestShapeInference) {
  const OpRegistrationData *reg;
  TF_CHECK_OK(OpRegistry::Global()->LookUp("FeatureMappingImport", &reg));
  OpDef op_def = reg->op_def;
  NodeDef def;
  std::vector<std::string> table_name_list = {"table_name1"};
  TF_CHECK_OK(NodeDefBuilder("dummy", &op_def)
